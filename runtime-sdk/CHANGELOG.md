...
- **Externalized API Configuration**
  - Moved default `ROOTLY_API_URL` from source code to build-time configuration
  - Added `.env` support for local development builds
  - Introduced custom build script to inject configuration during compilation
  - Prevents sensitive backend URLs from being exposed in public source control
  - Generated package still contains correct default URL for zero-config usage
